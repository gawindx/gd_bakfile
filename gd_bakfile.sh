...
function auth2 {
# inspired from https://github.com/soulseekah/bash-utils/tree/master/google-oauth2

if [ "$1" == "create" ]; then
	local RESPONSE=`curl --silent "https://accounts.google.com/o/oauth2/device/code" \
		--data "client_id=$CLIENT_ID&scope=$SCOPE"`
	local DEVICE_CODE=`prettyjson "$RESPONSE" | grep 'device_code' | \
		cut -d ':' -f 2 | sed 's/"//g' | sed 's/,//g' | sed 's/\ //g'`
	local USER_CODE=`prettyjson "$RESPONSE" | grep 'user_code' | \
		cut -d ':' -f 2 | sed 's/"//g' | sed 's/,//g' | sed 's/\ //g'`
	local URL=`prettyjson "$RESPONSE" | grep 'verification_url' | \
		cut -d ':' -f 2 | sed 's/"//g' | sed 's/\ //g'`
	URL="$URL:"`prettyjson "$RESPONSE" | grep 'verification_url' | \
		cut -d ':' -f 3 | sed 's/"//g' | sed 's/\ //g'`
	local TOKEN_TTL=`prettyjson "$RESPONSE" | grep 'expires_in' | \
		cut -d ':' -f 2  | sed 's/,//g'| sed 's/\ //g'`
	...
	DATA="$DATA&refresh_token=$REFRESH_TOKEN&grant_type=refresh_token"
...
function upload {
# Inspired from https://github.com/soulseekah/bash-utils/blob/master/google-drive-upload/upload.sh
...
local RESPONSE=`curl --silent "https://www.googleapis.com/drive/v2/files/root/children?q=$QUERY" \
	--header "Authorization: Bearer $ACCESS_TOKEN"`
local DESTDIR_ID=`prettyjson "$RESPONSE" | grep '"id":' | cut -d ':' -f 2  | sed 's/,//g'\
	| sed 's/\ //g' | sed 's/"//g'`
...
local RESPONSE=`curl --silent -X "POST" "https://www.googleapis.com/drive/v2/files" \
	...
	--header "Content-Length: $DATA_SIZE" \
	--data "$DATA_JSON"`
local DESTDIR_ID=`prettyjson "$RESPONSE" | grep -m 1 '"id":'| cut -d ':' -f 2 \
...
local RESPONSE=`curl --silent "https://www.googleapis.com/drive/v2/files/$GD_FILE_ID" \
	--header "Authorization: Bearer $ACCESS_TOKEN"`
local GD_FILE_NAME=`prettyjson "$RESPONSE" | grep "title" | cut -d '"' -f 4`
local GD_FILE_SIZE=`prettyjson "$RESPONSE" | grep "fileSize" | cut -d '"' -f 4`
...
local RESPONSE=`curl --silent "https://www.googleapis.com/drive/v2/files/$UPLOAD_DIR_ID/children?q=trashed%3Dfalse" \
	--header "Authorization: Bearer $ACCESS_TOKEN"`
local GD_FILE_LIST=`prettyjson "$RESPONSE" | grep "id" | cut -d '"' -f 4`
local count_index=0
local MY_ARRAY
